...
def process_with_classifier_via_script(image_path: str, filter_class: str, json_out: str) -> Dict[str, Any]:
...
def run_pipeline(foto, residue_name):
    """Ejecuta el flujo completo recibiendo una imagen numpy y el nombre del residuo.
    
    Args:
        foto: Imagen en formato numpy array (BGR)
        residue_name: Tipo de residuo ('lata', 'carton', 'botella')
    
    Returns:
        str: Ruta del archivo JSON con los resultados filtrados en mm
    """
    
    # Mapear nombre de residuo a filtros
    ...
    # Guardar la imagen numpy como archivo temporal
    ...
    # Retornar la ruta del archivo JSON con puntos filtrados y medidos
    resultado_final = os.path.join(PUNTOS_DIR, "temp_foto_puntos_filtrado_mm.json")
    print(f"\nFlujo completado. Resultado: {resultado_final}")
    with open(resultado_final,"+r") as f:
        data = json.load(f)
    return data
